...
Item {
  ...
  // pillcycle: "solid" (stock strip) -> "transparent" (stock transparent
  // strip, floating text) -> "pills" (pillbar capsules). Persisted on
  // bar.mode in shell.json; unknown from older configs means solid so a
  // fresh install lands fully visible. Hidden-parked is deliberately NOT a
  // cycle state: parked off-screen leaves no empty region to double-click
  // back (use SUPER+SHIFT+SPACE for that instead).
  property string barMode: "solid"
  readonly property bool pillMode: barMode === "pills"
  // Last shell.json transparent value, kept even when the cycle overrides
  // the strip; solid mode restores it from here.
  property bool requestedTransparentConfig: false
  ...
  function applyBarConfig() {
    ...
    requestedTransparentConfig = config.transparent === true
    var mode = String(config.mode || "solid").toLowerCase()
    if (mode !== "solid" && mode !== "transparent" && mode !== "pills") mode = "solid"
    barMode = mode
    applyModeTransparency()
  ...
  function setRequestedTransparency(value) {
    // Stock behavior (solid honors the shell.json transparent flag, the
  // transparent cycle state forces it on, pills force it off via
  // applyModeTransparency) incl. the wallpaper-contrast foreground swap.
  ...
  component BarPanel: PanelWindow {
    ...
    // Solid paints the stock strip (honoring the transparent flag);
    // pills never paint the strip — only per-widget capsules show.
    color: root.pillMode ? "transparent" : (root.transparent ? "transparent" : root.background)
  ...
  component ModuleSlot: Item {
    ...
    readonly property var registryComponent: {
      ...
      return w && w[registryName] ? w[registryName].component : null
    }
    ...
    // Pill backdrop. Painted first in pill mode so every widget sits inside
    // its own rounded capsule. Solid mode hides it (stock strip paints).
    // Widgets that draw their own pill (e.g. media mini-players) align with
    // the 2px inset; a widget can opt out with "pill": false on its entry.
    Rectangle {
      id: slotPill

      readonly property bool pillEnabled: slot.moduleSettings.pill !== false
      // Near-solid capsule of the theme bar background: the strip is always
      // transparent, so the pill itself is the widget's backdrop.
      readonly property color pillTint: Qt.rgba(
        root.background.r,
        root.background.g,
        root.background.b,
        0.88)

      visible: root.pillMode && pillEnabled && slot.width > 0 && slot.height > 0
      anchors.fill: parent
      anchors.topMargin: root.vertical ? 0 : Style.space(2)
      anchors.bottomMargin: root.vertical ? 0 : Style.space(2)
      anchors.leftMargin: root.vertical ? Style.space(2) : 0
